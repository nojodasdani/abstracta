...
function errorlocation() {
  myApp.alert('Necesitamos tener acceso a tu ubicación con el fin de recomendarte entretenimiento cercano a ti', 'Abstracta');
}
...
myApp.onPageInit('contacto', function (page) {
  $$("#myform").submit(function(e) {
    ...
    $$.ajax({
      method: "POST",
      url: "http://apps.opion-tech.com/correoAbstracta.php",
      ...
      success: function(response) {
        myApp.alert("El correo se envió exitosamente", "Abstracta");
        $$("#myform")[0].reset();
      },
      error: function(error) {
        myApp.alert("Ocurrió un error, intenta de nuevo", "Abstracta");
      },
      complete: function() {
        $$("#loading").remove();
        $$("#myform").show();
